...
class ReceptionRecord(Base):
    ...
    scenario_name = Column(String, nullable=False)  # 场景名称
    date = Column(String)  # 招待日期
    location = Column(String)  # 招待地点
    org = Column(String)  # 招待对象单位
    guests = Column(Integer, default=0)  # 招待对象人数
    accompany_count = Column(Integer, default=0)  # 陪同人数
    per_capita = Column(Float, default=0.0)  # 人均餐标
    meal_total = Column(Float, default=0.0)  # 餐费合计
    alcohol_total = Column(Float, default=0.0)  # 酒水合计
    gift_total = Column(Float, default=0.0)  # 纪念品合计
    room_total = Column(Float, default=0.0)  # 住宿合计
    total_amount = Column(Float, default=0.0)  # 总金额
    reason = Column(Text)  # 招待事由
    submitter = Column(String)  # 提交人
    status = Column(String, default="待审核")  # 待审核 / 已通过 / 已驳回
    remark = Column(Text)  # 审核备注
    created_at = Column(String, default=lambda: datetime.now().isoformat())
