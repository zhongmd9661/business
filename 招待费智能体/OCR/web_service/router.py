...
async def upload_files(
    ...
    # Save files — directory: uploads/20260723_143045_{task_id}
    task_dir = UPLOAD_DIR / f"{ts}_{task.id}"
...
async def _process_review_task(task_id: int):
    ...
    try:
        ...
        # Reconstruct batch dir from created_at timestamp: uploads/20260723_143045_{task_id}
        try:
            created = datetime.fromisoformat(task.created_at)
            ts = created.strftime("%Y%m%d_%H%M%S")
        except (ValueError, TypeError):
            ts = "unknown"
        batch_dir = UPLOAD_DIR / f"{ts}_{task_id}"
