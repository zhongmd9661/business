...
class OutputOrganizer:
    ...
    def reorganize(raw_dir: Path, target_dir: Path, seq: int, stem: str, batch_summary: str = None) -> Path:
        ...
        # 收集图片到 images 子目录
        _collect_images(md_dir, results_dir)
...
def _move_middle_json(search_dir: Path, stem: str, results_dir: Path, seq: int) -> None:
    """查找并移动 MinerU 的中间 JSON 文件到识别结果目录的 _json 子文件夹。
    ...
    for candidate in json_candidates:
        ...
        if src.exists():
            dst = json_dir / f"{seq}_{stem}.json"
            shutil.copy2(src, dst)
            logger.info(f"  -> _json/{dst.name}")
            return
...
def _collect_images(search_dir: Path, results_dir: Path) -> None:
    """收集 MinerU 解析生成的图片到识别结果目录的 images 子文件夹。"""
    images_dir = results_dir / "images"
    images_dir.mkdir(parents=True, exist_ok=True)
    image_suffixes = {".jpg", ".jpeg", ".png", ".webp", ".bmp", ".gif", ".svg"}
