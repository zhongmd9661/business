...
- [ ] 3.7 实现输入目录 zip 文件自动解压（统一 UTF-8 编码）
- [ ] 3.8 实现运行前 prompt 用户输入批次摘要，归档目录为 `data/已识别/<摘要>_YYYYMMDD_HHMMSS/`
...
- [ ] 5.1 输出目录扁平化：移除 `auto/` 中间层，结构改为 `{timestamp}/{seq}_{filename}/` 和 `{timestamp}/{seq}_{filename}/images/`
- [ ] 5.2 输出目录和文件名带序号：输入 `001_xxx.pdf` → 输出 `001_xxx/001_xxx.md`
- [ ] 5.3 无序号文件自动分配序号
- [ ] 5.4 每个 Markdown 输出增加文档摘要（`## 摘要` 章节）

## 6. 待完善

- [ ] 6.1 单个文件失败时记录错误并继续处理
