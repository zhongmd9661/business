...
- 输入文件带序号前缀（`001_`、`002_`...）
- 输出目录带时间戳（`data/已识别/{摘要}_YYYYMMDD_HHMMSS/`，含 `源文件/` 和 `识别结果/`）
...
**Goals:**
- 批量解析 `data/input/` 下的文档，输出 Markdown 文本
- 支持 PDF、图片、Office 文档（docx、xlsx、pptx）格式输入
- 自动检测扫描版 PDF 并启用 OCR
- Office 文档通过 MinerU 内置 `office_docx_analyze`、`office_xlsx_analyze` 转换为 Markdown
- 输出带时间戳的目录，方便对比多次运行结果
- 输入文件带序号前缀，便于追踪和排序
- 解析过程显示进度、耗时、汇总
- 源文件与识别结果分目录存放于 `data/已识别/{摘要}_{timestamp}/`
...
### 2. 输出目录: 批次子目录
- **决策**: 每次运行在 `data/已识别/` 下创建 `{摘要}_{YYYYMMDD_HHMMSS}` 子目录，内含 `源文件/` 和 `识别结果/`
- **理由**: 源文件与识别结果集中管理，文件名一致便于对应，多次运行结果可对比
